export default {
  ...
  home: {
    ...
    banner: {
      flag: "NEW",
      title: "Stack profiles are here",
      copy: "Conventions now live in stacks/ profiles, detected for the task at hand — no more one-size-fits-all rules.",
...
<dialog ref={dialogRef}>
  ...
  <form method="dialog">
    ...
    agnostic: {
      eyebrow: "Agnostic by design",
      title: "Same philosophy. Your stack, your host, your agent.",
      copy: "Lexis-Two doesn't lock you into a framework, an editor, or an orchestrator. The core stays universal; the specifics are detected per task and per host.",
      cards: [
        { tag: "Stack", title: "Stack-agnostic", desc: "Philosophy lives in AGENTS.md. Conventions live in stacks/<id>.md and are detected from the task's files — Node/TS shipped, Python/Go/Rust/Astro planned." },
        { tag: "Host", title: "Host-agnostic", desc: "One skills/ catalog and thin adapters across OpenCode, Claude Code, Cursor, Codex, Gemini CLI, Copilot, pi, and plain AGENTS.md." },
        { tag: "Agent", title: "Agent-agnostic", desc: "A portable ruleset, not a product lock-in. Any agent that reads instructions can follow the loop without a private orchestrator." },
      ],
    },
    benchmarks: {
  ...
  commands: {
    ...
    items: [
      ...
      { name: "/lexis audit (or /lexis a)", desc: "Audits the entire repository for dead code, unnecessary dependencies, or premature abstractions — stack-aware where a profile exists." },
      ...
      { name: "/lexis security (or /lexis s)", desc: "Runs a focused security audit. Uses the stack audit when a profile exists; otherwise generic checks." },
  ...
  adapt: {
    title: "Add a stack",
    subtitle: "New ecosystem? Add one profile file and a detection row — no new framework.",
    cards: [
      { title: "1. stacks/<id>.md", desc: "Copy <code>stacks/_template.md</code> and fill detection, tools, and conventions." },
      { title: "2. Detection table", desc: "Add one row to <code>AGENTS.md</code> so agents know when to load the profile." },
      { title: "3. npm test", desc: "Re-syncs the four instruction-tier copies and catches drift." },
    ],
  },
  stacks: {
    title: "Stack profiles",
    subtitle: "Philosophy stays agnostic; conventions live in stacks/<id>.md and are detected per task.",
    items: [
